...
class NewsPredictor:
    ...
    def prepare_predictor(self,df):
        self.df=df
        #count based vectors
        self.count_vect = TfidfVectorizer(analyzer = DataAnalyzer().custom_analyzer)
    ...
    def predictById(self,id):
        ...
        for news in sorted_similar[1:10]:
            index=int(news[0])
            print(index)
...
class Server(SimpleHTTPRequestHandler):
    ...
    def do_HISTORY(self):
        ...
        news=newsdb.get_news_from_id(id)
        history.append(news)
        print(news['title'])
        print("-------")
    ...
    def do_RECOMMENDATION(self):
        ...
        predicted=predictor.predictById(id)
        lists=[]
        for __ in predicted:
            lists.append(__)
